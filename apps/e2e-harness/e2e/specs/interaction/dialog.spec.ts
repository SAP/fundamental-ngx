...
test.describe('Dialog Interaction', () => {
    ...
    test('closes dialog via close button', async ({ page }) => {
        ...
        const dialog = page.locator('.cdk-overlay-container fd-dialog, .cdk-overlay-container [role="dialog"]').first();
        await expect(dialog).toBeVisible();
        const closeBtn = dialog.locator('button[aria-label="close"], button[glyph="decline"], .fd-dialog__decisive-button').first();
        if (await closeBtn.count() > 0) {
            await closeBtn.click();
        } else {
            const cancelBtn = dialog.locator('button').filter({ hasText: /cancel|close|no/i }).first();
