    ...
    return makeEnvironmentProviders([
        provideEnvironmentInitializer(() => {
            for (const lang of langs) {
                ...
                // Match on the (string) locale, not object identity, so a cloned
                // or patched Chinese FdLanguage still gets its script aliases.
                const loc = lang.locale?.toLowerCase();
                if (loc === 'zh-hans') {
                    registerLanguage(lang, 'zh-hans', 'zh');
                } else if (loc === 'zh-hant') {
                    registerLanguage(lang, 'zh-hant');
                }
            }
        })
    ]);
