...
@Component({
    ...
    template: ` <fd-calendar
        ...
        <br />
        <div>Selected First Date: {{ $safeNavigationMigration(selected.start?.toDate()) | date: 'shortDate' }}</div>
        <br />
        <div>Selected Last Date: {{ $safeNavigationMigration(selected.end?.toDate()) | date: 'shortDate' }}</div>`,
    providers: [
        {
