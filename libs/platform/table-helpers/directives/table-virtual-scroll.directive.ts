...
export class TableVirtualScrollDirective extends TableVirtualScroll implements OnChanges, OnInit, OnDestroy {
        ...
        if (this.scrollWholeRows) {
            if (!this._focusableGridSubscription) {
                this._focusableGridSubscription = this._table._focusableGrid.itemFocused.subscribe((event) => {
                    // need to put this in a setTimeout so it happens after the _keydownHandler is executed
                    setTimeout(() => {
    ...
    _keydownHandler(event: KeyboardEvent): void {
        if (this.scrollWholeRows) {
            if (this._focusedCell?.rowIndex === 1 && KeyUtil.isKeyCode(event, UP_ARROW)) {
                if (this._table.tableContainer.nativeElement.querySelectorAll('tr')[1].ariaRowIndex !== '1') {
                    const cellToFocus = {
     ...
     * state scrollTopPosition.
     */
    private _setRows(startingNode: number): void {
        this._table.setCurrentlyRenderedRows(startingNode, this._getNumberOfRowsToDisplay());
