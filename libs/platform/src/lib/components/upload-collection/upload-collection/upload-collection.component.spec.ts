...
describe('UploadCollectionComponent', () => {
    ...
    it('toolbar: should create new folder', waitForAsync(() => {
        ...
        const items = [
            ...component.dataSource.dataProvider.items,
            newFolderData
        ];
        ...
        const newFolder = component._getList().find((item) => item.documentId === newTemporaryFiles.documentId);
        expect(newFolder.name).toEqual(newFolderName);
    }));

    it('toolbar: should remove item', waitForAsync(() => {
        const item = component.dataSource.dataProvider.items.pop();
        component._activeItem = item;

        spyOn(component.dataSource, 'delete').and.returnValue(of(null));
    ...
    it('list: should update file version', waitForAsync(() => {
    ...
    it('list: should upload files', waitForAsync(() => {
        const file1: MockFile = new File([''], 'file1');
        spyOnProperty(file1, 'size').and.returnValue(1024);
        const file2: MockFile = new File([''], 'file2');
        spyOnProperty(file2, 'size').and.returnValue(1048580);
    ...
    it('list: should show items of page 2', waitForAsync(() => {
