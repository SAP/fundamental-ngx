...
import { PopoverFillMode, MenuKeyboardService, MenuItemDirective } from '@fundamental-ngx/core';
...
export class SelectComponent implements OnChanges, AfterContentInit, OnDestroy, ControlValueAccessor {
    ...
    @Output()
    readonly itemClicked: EventEmitter<OptionItem> = new EventEmitter<OptionItem>();
    ...
    get triggerValue(): string {
        return this.selectedItem ? this.selectValue : this.placeholder;
    }
    ...
    onMenuClickHandler(index: number) {
        console.log('selected value 1' + JSON.stringify(this.isSelected));
        this.selectedItem = this.dropdownValues[index];
        this.isSelected = [];
        console.log('selected value 1' + JSON.stringify(this.isSelected));
        if (this.selectedItem) {
            this.isSelected[index] = true;
            console.log('selected value 1' + JSON.stringify(this.isSelected));
            console.log('selected value 2' + JSON.stringify(this.selectedItem));
            this.handleClickActions(this.selectedItem);
            this.itemClicked.emit({ item: this.selectedItem, index: index });
        }
    }
    ...
    private handleClickActions(term): void {
        ...
        if (this.fillOnSelect) {
            this.selectValue = this.displayFn(term);
        }
    }
    /** @hidden */
