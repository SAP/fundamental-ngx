...
export class OptionComponent implements OnInit {
    ...
    get viewValueText(): string {
        return this.viewValue ? this.viewValue : ((this.elRef.nativeElement as HTMLElement).textContent || '').trim();
    }

    /** Returns the view value text of the option, or the viewValue input if it exists. */
    setSelected(value: boolean, fireEvent: boolean = true): void {
        console.log('inside optin component setSelected value');
        this.selected = value;
    ...
    selectionHandler(): void {
        if (!this.selected && !this.disabled) {
            this.selected = true;
            console.log('inside optin component selectionHandler value' + this.selected);
            this.selectedChange.emit(this);
        }
    }*/
}
