...
describe('add-theming schematic', () => {
    ...
    describe('angular.json modifications', () => {
        ...
        it('should add icon font styles', async () => {
            ...
            const styles = (tree.readJson('./angular.json') as Record<string, any>).projects.test.architect.build.options.styles;
            iconFonts.forEach((font) => {
                expect(styles.find((jsonStyle: any) => typeof jsonStyle === 'object' && jsonStyle.bundleName === font)).toBeDefined();
            });
        });
    });
    ...
    describe('standalone app', () => {
        ...
        it('should correctly take the theme name into the configuration', async () => {
            ...
            expect(tree.readText('./test/src/app/app.config.ts'))
                .toContain('provideTheming({ defaultTheme: \'sap_fiori_3\', changeThemeOnQueryParamChange: false })');
        });
        it('should correctly configure `readThemeFromURL` option', async () => {
            tree = await runner.runSchematic('add-theming', { project: 'test', theme: 'sap_fiori_3', readThemeFromURL: true }, tree);
            expect(tree.readText('./test/src/app/app.config.ts'))
                .toContain('provideTheming({ defaultTheme: \'sap_fiori_3\', changeThemeOnQueryParamChange: true })');
    ...
    describe('non-standalone app', () => {
        ...
        it('should correctly take the theme name into the configuration', async () => {
            tree = await runner.runSchematic('add-theming', { project: 'test', theme: 'sap_fiori_3' }, tree);
            expect(tree.readText('./test/src/app/app.module.ts'))
                .toContain('provideTheming({ defaultTheme: \'sap_fiori_3\', changeThemeOnQueryParamChange: false })');
        });
        it('should correctly configure `readThemeFromURL` option', async () => {
            tree = await runner.runSchematic('add-theming', { project: 'test', theme: 'sap_fiori_3', readThemeFromURL: true }, tree);
