...
describe('FeedListItemComponent', () => {
  ...
  it('should render author title with link', () => {
    ...
    fixture.detectChanges();
    expect(component.elementRef().nativeElement.querySelector('.fd-feed-list__name a').innerText).toContain(authorTitle);
    expect(component.elementRef().nativeElement.querySelector('.fd-feed-list__name a').getAttribute('href')).toEqual(authorLink);
  });
  ...
  it(`should label's more and less buttons`, async () => {
    ...
    let text = component.elementRef().nativeElement.querySelector('.fd-feed-list__link--more').innerText.toLowerCase();
