...
@Component({
    ...
    providers: [{ provide: FD_OPTION_PARENT_COMPONENT, useExisting: SelectComponent }]
})
export class SelectComponent extends _MatSelectMixinCommons
    ...
    /**
     * Time to wait in milliseconds after the last keydown before focusing or selecting option based on alphanumeric
     * keys.
     */
    @Input()
     ...
     *
     * Todo: Extract this method out of here or inject SelectMobileComponent as type into this component. Ideally
     * select does not need to be aware of SelectMobileComponent type. We could create provider called
     * MobileStrategy that would have a methods adn one of them woudl return Impl type. + it can have initialize where
     * we would hide this code.
     *
     * @hidden
