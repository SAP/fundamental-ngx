...
export class InfoLabelComponent implements HasElementRef {
     ...
     * @default 7
     */
    readonly color = input<InfoLabelColorInput | null | undefined>(7);
    ...
    protected readonly _validatedColor = computed(() => {
        const numericColor = Number(this.color() ?? 7);
        ...
        return 7; // Default fallback for invalid values
    });
    ...
    protected readonly _cssClass = computed(() =>
        [
            ...
            `fd-info-label--accent-color-${this._validatedColor()}`
        ]
            .filter(Boolean)
            .join(' ')
    );
