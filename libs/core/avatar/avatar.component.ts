...
export class AvatarComponent {
    ...
    /** @hidden */
    protected readonly _content = viewChild<ElementRef>('content');

    /** @hidden Internal state for background image */
    protected readonly _bgImage = signal<string | null | undefined>(null);

    /** @hidden Internal state for computed abbreviation */
    protected readonly _abbreviate = signal<string | null | undefined>(null);
    ...
    protected readonly _role = computed(() => (this.zoomGlyph() || this.clickable() ? 'button' : 'img'));

    /** @hidden If a default placeholder should be displayed */
    protected readonly _showDefault = computed(
        () =>
            // Show default if explicitly requested OR if no content is available
            this._shouldShowDefaultIcon() ||
            (!this._abbreviate() && !this._imageLoaded() && !this.glyph() && !this.image())
    );
    ...
    private _verifyImageUrl(srcValue: string, onErrorCallback: () => void): void {
        ...
        if (this._bgImage() === newBgImage && this._imageLoaded()) {
            return;
        }
