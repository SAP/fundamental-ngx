...
we use the git commit messages to **generate the Fundamental change log**.
...
if you look at the format of our npm packages you will see following:

`0.11.0-rc.10` where you can read this as `MAJOR` . `MINOR` . `PATCH PR-BUILD`.
  
and depending on the PR title we increment one of these numbers. 
  * This is happening as part of the CI process that reads your **PR Title (not commit message)** and decide which number to increment. 
...
The PR check process fails if you put something else then `WIP|feat|chore|test|doc|fix`
